...
8. Confirm the file includes test placement rules for the repository's backend, frontend, or stack-specific test surfaces.
9. Confirm the test placement rules state that TDD tests are retained as behavior specifications unless their signal is duplicated, obsolete, implementation-coupled, or intentionally moved to a better owner/layer.
10. Confirm the test-related script check is wired into an actual enforcement path such as `scripts/check-consistency.sh`, git hooks, or CI when the repository already has such guardrails.
11. If no enforcement path exists yet, state that explicitly instead of implying the prompt text alone is enough.
12. State whether documentation impact exists. For creating or editing `AGENTS.md`, the file itself is the documentation update.
13. Confirm the project root keeps `CLAUDE.md -> AGENTS.md`, or explicitly state that the repository does not use Claude/Codex auto-discovery.
