...
- `docs/architecture/`：当前服务边界和数据归属决策。
  ...
  - `docs/architecture/go-service-design.md`：Go 服务内分层、application 文件组织、typed DTO / `map[string]any` 使用、运行时依赖映射、PostgreSQL repository SQL 落点、命名和映射归属、migration、缓存和事件规则。
