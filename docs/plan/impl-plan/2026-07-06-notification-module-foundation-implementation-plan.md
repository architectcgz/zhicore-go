...
**目标：** 把 `zhicore-notification` 从占位模块推进到拥有站内收件箱、聚合未读、已读操作、runtime、交互通知 consumer、偏好 / DND / delivery 和活跃受众 campaign fanout 基础的可交付模块。

**架构：** Notification 拥有通知收件箱、group state、未读数、偏好、免打扰、作者订阅、campaign、delivery 和 realtime fanout 语义；源用户、文章、评论、私信和榜单事实仍由来源服务拥有。站内通知写入是权威事实，WebSocket / Email / SMS 只影响 delivery 状态，不回滚 inbox。发文 campaign 第一阶段只对 User contract 返回的 `HOT` / 活跃受众物化 inbox，`WARM` / `COLD` 不提前增加 `notification_stats`。
...
- User follower shard contract 当前已具备基础形态，但还需要补 `audienceClass` / `activeSince` 活跃受众筛选；campaign worker 不能按全量粉丝 fanout 开工。

## 不可并行修改文件

- `libs/contracts/clients/user/contract.go`：Message 和 Notification 都会追加 User provider-owned internal contract；两者不是业务依赖，执行时只需按能力拆分或协调同文件冲突，不能把 Notification 设计成依赖 Message 模块。
...
- [x] migration 创建 `notifications`、`notification_group_state`、`notification_stats`、`consumed_events`。
...
- [x] 未读总数和分类 breakdown 优先读 `notification_stats`，不在热路径扫描 `notifications`。
...
**验收清单：**
- [x] 必填配置覆盖 Postgres、Redis、RabbitMQ、`public_id.active_version`、`public_id.secrets`、`consumer.consumed_events_retention`、`realtime_fanout.timeout`、campaign claim / batch 参数。
- [x] 配置日志和 error 不泄露 public ID secret、DSN、RabbitMQ URL、Redis credential。
- [x] `/health/live` 不探下游。
- [x] `/health/ready` 检查 Postgres、Redis、RabbitMQ 和 enabled worker descriptor；后续 consumer / campaign worker 增加时必须同步更新 runtime wiring、start / stop、panic recovery、readiness descriptor 和配置校验。
- [x] 启动路径不自动执行 migration。

- [x] **步骤 1：写 config / health / lifecycle 失败测试**
- [x] **步骤 2：实现 runtime 和 server**
- [x] **步骤 3：运行 runtime 验证**
...
**验收清单：**
- [x] 消费 `content.post.liked`、`comment.created`、`user.followed`。
- [x] `eventId` 和 `dedupeKey` 双层幂等均落库。
- [x] duplicate event ack no-op；数据库事务提交成功后才 ack；已判定 no-op 的自交互 ack；临时依赖错误 nack / requeue；producer contract 错误写入 DLQ 后 ack；fanout 失败不 nack。
- [x] 自己赞自己、自己关注自己、自己回复自己 no-op。
- [x] `comment.created` 缺 `postAuthorId` / `parentAuthorId` 等必需字段时判 producer contract 错误并 retry / DLQ，不在高频路径静默补查。
- [x] 本任务 fanout 只做站内 inbox 写入后的 WebSocket / unread hint best-effort，不执行 Email / SMS / digest delivery，不应用偏好 / DND；偏好、DND 和 delivery ledger 在任务 5 后才生效。
- [x] 交互通知的 actor 展示首期只使用事件 payload 中的 actor ID / snapshot；本任务不新增 User summary contract，若需要实时用户摘要必须另开 User provider-owned 前置切片。
- [x] DLQ envelope 至少包含 `eventId`、`eventType`、`routingKey`、`consumer`、`errorClass`、`failedAt`、`retryCount`、`payloadHash`，不得包含正文全文、raw token 或 credential。
- [x] realtime fanout 是 best-effort，失败不回滚 inbox。
- [x] consumer 名称和队列与 `deploy/docker/rabbitmq/definitions.json` 对齐。

- [x] **步骤 1：写 consumer、idempotency、fanout 失败测试**
- [x] **步骤 2：实现 application consumer 和 RabbitMQ adapter**
- [x] **步骤 3：运行 consumer 验证**
...
**验收清单：**
- [x] 最终 path 明确为：`GET /api/v1/notification-preferences`、`PUT /api/v1/notification-preferences`、`GET /api/v1/notification-dnd`、`PUT /api/v1/notification-dnd`、`GET /api/v1/author-subscriptions/{authorId}`、`PUT /api/v1/author-subscriptions/{authorId}`、`GET /api/v1/notification-deliveries`、`POST /api/v1/notification-deliveries/{deliveryId}/retry`；如保留 `/api/v1/notifications/preferences` 等历史 alias，必须在 endpoint 文档中标注 alias 和 canonical path。
- [x] `SMS` 第一阶段禁止启用。
- [x] DND 校验 `startTime != endTime`，并写清跨日窗口、timezone、categories、channels 语义。
- [x] 作者订阅固定 `ALL`、`DIGEST_ONLY`、`MUTED` 及 channel boolean 规范化。
- [x] delivery retry 区分本人重试自己的记录和管理员重试任意记录。
- [x] provider unconfigured 只影响 delivery 状态，不影响站内通知真相源。
- [x] migration up/down 可通过 `golang-migrate` 往返验证；偏好 / delivery 表回滚不删除 inbox 核心表。

- [x] **步骤 1：写 handler / app / repo / permission 失败测试**
- [x] **步骤 2：实现偏好、DND、订阅和 delivery ledger**
- [x] **步骤 3：运行验证**
...
## 任务 6：发文 campaign fanout 与 active follower shard contract
...
**验收清单：**
- [x] `content.post.published` 只能创建 campaign 和初始 `HOT` audience shard，不能同步逐粉丝写库。
- [x] 确认 Notification 不依赖 Message；如 Message 计划并行修改 User contract，仅协调同文件冲突，不能把 Message 合并作为业务前置条件。
- [x] 执行前确认任务 5 已合并；campaign fanout 必须读取偏好、DND、作者订阅和 delivery 规则，不允许绕过 active delivery 策略。
- [x] shard claim 使用 `FOR UPDATE SKIP LOCKED` 或等价语义。
- [x] claim timeout / batch size 从配置读并受上下界约束。
- [x] `ListFollowerShard` 请求必须携带 `audienceClass` / `activeSince`；发文 fanout 默认 `HOT`，不得把 `ALL` 作为普通发布路径。
- [x] `ListFollowerShard(HOT)` degraded 时 retry / DLQ，不能把空结果当成功，不能自动 fallback 到 `ALL`。
- [x] delivery 状态至少区分 `IN_APP`、`WEBSOCKET_PENDING`、`DIGEST_PENDING`、`SKIPPED`。
- [x] group-state rebuild 锁和单用户 rebuild 路径可验证。
- [x] campaign worker 纳入 runtime start / stop、panic recovery、readiness descriptor 和配置校验。
- [x] migration up/down 可通过 `golang-migrate` 往返验证；campaign 表回滚不删除 inbox / preference / delivery 表。
- [x] race 验证覆盖并发 shard claim、重复 event、read-all 与 fanout 同时更新 unread。

- [x] **步骤 1：写 follower shard contract 和 campaign 失败测试**
- [x] **步骤 2：实现 User active follower contract、campaign domain、repository 和 worker**
- [x] **步骤 3：运行验证**
