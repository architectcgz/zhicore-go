...
- [ ] `ListFollowerShard` 请求必须携带 `audienceClass` / `activeSince`；发文 fanout 默认 `HOT`，不得把 `ALL` 作为普通发布路径。
- [ ] `ListFollowerShard(HOT)` degraded 时 retry / DLQ，不能把空结果当成功，不能自动 fallback 到 `ALL`。
