...
部分已实现服务仍把命名的 JSON request / response payload 类型放在 `handler.go` 中，导致 handler 同时承担路由流程、参数校验、application 映射和 payload 类型归属，后续新增 endpoint 时容易继续膨胀。

当前检查结果：

- `zhicore-comment`：`handler.go` 中仍有 `createCommentReq`、`createCommentResp`、分页响应、评论项响应、删除 / 点赞响应等命名 payload 类型。
- `zhicore-file`：`handler.go` 中仍有 `UploadFileResp`。
- `zhicore-user`：`handler.go` 中仍有 `userProfileResp`、`relationshipPageResp`。
