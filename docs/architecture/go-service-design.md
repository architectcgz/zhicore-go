...
已知 schema 的 HTTP request / response、application command / query / result、ports input / output、provider client DTO、event payload、parser input 和持久化映射默认使用 typed struct，不用 `map[string]any` 或 `map[string]interface{}` 表达业务契约。`map[string]any` 只允许用于真正开放的 provider payload、测试里快速构造 / 断言 JSON、短生命周期边界适配、SQL 参数容器（例如 `...any` / `[]any`）或明确的探索 spike。若动态 map 出现在 parser / validator / decoder 热路径、公开 contract、事件 payload、application API 或跨层 DTO 中，review 默认要求改成 typed struct；未知字段优先用 typed struct 加 `json.RawMessage`、discriminator 或显式扩展字段承接。确需保留动态 map 的生产路径，必须在代码或设计文档说明开放字段 owner、允许的 key 范围、校验边界和性能影响；热路径还要用 `benchmem` 关注 `B/op` 和 `allocs/op`。

HTTP 入站层文件组织默认把 JSON request / response payload 类型放在 `api/http/payloads.go`；当单个服务的 HTTP 面继续变大时，可按 API family 拆成 `<family>_payloads.go`，例如 `auth_payloads.go`、`post_payloads.go`、`admin_payloads.go`。`handler.go` 只保留路由注册、handler 流程、参数校验调用、application command/query 映射和必要 helper，不继续堆积服务级 `Req` / `Resp` 类型。少量仅服务单个 handler、没有复用价值的函数内匿名 decode struct 可以保留在函数内；一旦它成为命名类型、被测试引用、被多个 handler 复用，或同文件 payload 类型持续增长，就必须移入 payload 文件。不要把该文件命名为 `schema.go`：本仓库中 schema 通常指 HTTP contract 文档或数据库 migration 事实源，容易和 `services/<service>/api/http/README.md`、`endpoints/` 及 `migrations/` 混淆。

Application 文件组织默认按 use case / concern 拆分，不再把多个命令、查询、事件映射和 helper 堆进一个超长 `service.go`。`service.go` 只保留 `Dependencies` / `Deps`、`Service`、`NewService`、基础错误或常量等服务骨架；公开 command / query / result DTO 放 `dto.go` 或既有 DTO 文件；具体 use case 放 `<action>.go`，例如 `create_comment.go`、`publish_post.go`、`profile_commands.go`；事件 outbox 映射放 `events.go`；缓存、分页、mapper、guard、body validation 等横切 concern 放对应窄文件。新增 application 能力时应直接落到这些窄文件或同风格新文件，避免先写成单体 `service.go` 再事后拆分。
...
PostgreSQL repository 的复杂 SQL 默认放在 `services/<service>/internal/<domain>/infrastructure/postgres/sql/*.sql`，由同包 `sql.go` 使用 `//go:embed sql/*.sql` 加载为包级变量。Go repository 文件只保留事务编排、参数映射、`Scan` / `Exec` 调用和底层错误翻译，不把稳定业务查询、审查重点 SQL 或多行 SQL 混进 Go 流程代码。

以下 SQL 应优先外置：CTE、`INSERT` / `UPDATE ... ON CONFLICT`、`FOR UPDATE SKIP LOCKED`、聚合 / 列表查询、事务关键写入、跨多表查询，以及任何需要 reviewer 逐行审查执行计划或幂等语义的多行语句。极短、一次性、不会成为稳定契约的简单 SQL 可以内联，但新增 repository 代码遇到复杂查询时默认采用外置 SQL；既有内联 SQL 可在聚焦重构中迁移，不混入无关功能提交。
