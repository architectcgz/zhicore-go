...
存储层默认禁止把 SQL 语句硬编码在 Go 代码里。PostgreSQL repository 的 SQL 默认放在 `services/<service>/internal/<domain>/infrastructure/postgres/sql/*.sql`，由同包 `sql.go` 使用 `//go:embed sql/*.sql` 加载为包级变量。Go repository 文件只保留事务编排、参数映射、`Scan` / `Exec` 调用和底层错误翻译，不把业务查询、审查重点 SQL 或多行 SQL 混进 Go 流程代码。CTE、`INSERT` / `UPDATE ... ON CONFLICT`、`FOR UPDATE SKIP LOCKED`、聚合 / 列表查询、事务关键写入、跨多表查询，以及任何需要 reviewer 逐行审查执行计划或幂等语义的语句，一律外置，不得内联。
