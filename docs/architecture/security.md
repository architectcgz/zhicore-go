...
- 用户身份、登录凭证、角色、启用 / 禁用状态。
- JWT 签发、refresh、失效语义和用户状态变更后的 token 处理规则。
- 用户密码校验、密码 hash、凭证更新和登录失败错误映射。
...
| 类型 | Owner | 示例 |
| --- | --- | --- |
| 登录态和角色 | Gateway / User | 未登录、token 过期、管理员角色 |
| 资源归属和可见性 | 资源归属服务 application | 文章作者才能编辑草稿、评论作者才能删除评论 |
| 跨服务关系事实 | 事实归属服务 contract | 拉黑关系、陌生人私信设置、用户禁用状态 |
...
- 登录、注册、refresh、JWT 校验、token 黑名单、用户禁用和角色变更。
