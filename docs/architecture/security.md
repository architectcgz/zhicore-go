...
`zhicore-file` 是统一文件入口和文件生命周期 owner，但不拥有业务实体和文件关联关系。
...
- `GET /api/v1/files/{fileId}/url` 返回的 URL 不能泄漏对象存储签名参数到日志。
- 删除文件需要考虑 metadata 状态变更和对象存储删除的补偿；业务服务删除实体成功但文件删除或引用释放失败时必须有补偿或技术债记录。
...
- File / 外部 URL 相关测试必须覆盖非法类型、超限、清理路径和 SSRF 防护边界。
