...
- 调用 File service 处理封面或正文图片相关的文件校验、URL 解析和删除行为。
...
- 评论 ID、点赞计数异步 delta 台账、顶级评论 HOT 排序读模型和默认 RECOMMENDED 排序读模型。
...
- 调用 User 获取作者摘要。
- 调用 File service 处理评论媒体文件校验、URL 解析和删除。
...
- 默认使用 Admin 数据库 identity 生成举报和审计内部 ID；只有重新启用集中发号策略后，才允许调用 IdGenerator。
...
- 文件 ID、文件元数据、已上传对象资源、对象存储 key、访问级别、文件 URL 解析、签名策略、文件删除和生命周期清理。

拥有的存储：

- 对象存储资源。
- File 服务本地文件元数据和生命周期状态。

权威查询：

- 文件 URL 查询。
- 文件存在性、类型、访问级别、引用可用性和访问检查。

Provider contract：

- `libs/contracts/clients/file`
- `libs/contracts/events/file`，仅当文件生命周期事件成为跨服务事实时使用。

允许依赖：

- 正常上传和 URL 查询不应依赖业务服务。
- MinIO / 对象存储是 File 服务的基础设施依赖，不是另一个业务服务 owner。

说明：

- File service 不拥有头像、封面、评论媒体或文章附件的业务绑定关系；这些引用关系仍由对应业务服务拥有。
- 业务服务只保存 `fileId`，不能把 CDN URL、对象存储 key、签名 URL 或 URL 生成规则保存为长期事实。
...
- 当前默认 ID 策略见 `docs/architecture/id-strategy.md`：内部主键使用各服务数据库 `BIGINT GENERATED BY DEFAULT AS IDENTITY`；外部公开 ID 使用独立 `public_id`、`public_no` 或 `order_no`。
...
| File | 对象存储资源、File 本地文件元数据和生命周期状态 |
...
| 解析文件 URL | File | 其他服务保存 file ID 并调用 File service |
...
| 文章发布、更新、删除、可见性变化、标签更新、点赞、取消点赞、收藏、取消收藏、浏览 | Content | `content.post.published`, `content.post.updated`, `content.post.deleted`, `content.post.visibility_changed`, `content.post.tags.updated`, `content.post.liked`, `content.post.unliked`, `content.post.favorited`, `content.post.unfavorited`, `content.post.viewed` | `ZhiCore-post-events` |
...
- 对 File service 托管的系统内文件，只保存 `file_id` / `*FileId` 引用；可展示 URL 由 File service 在读取时解析或缓存派生。
...
| 副本 | 位置 | 一致性 | 用途 |
|------|------|--------|------|
| `comment_post_stats.total_comments` | Comment 服务 | 强一致（事务内维护） | Comment 分页查询返回的 `totalComments` |
| `post_stats.comment_count` | Content 服务 | 最终一致（消费 Comment 事件更新） | 文章列表、详情页展示的评论计数 |

规则：

- 文章详情和列表 API 的 `commentCount` 字段使用 Content 拥有的 `post_stats.comment_count`（最终一致）。
- 评论列表 API 的 `totalComments` 字段使用 Comment 拥有的 `comment_post_stats.total_comments`（强一致）。
- 前端不应假设两者实时相等；短暂差异（通常 < 1s）是已知的最终一致窗口。
- Content 的 `comment_count` 通过消费 `comment.created` / `comment.deleted` 事件维护，不得直接调用 Comment 查询计数。

## 开放决策

实现相关切片前必须解决：

- File metadata schema、MinIO adapter、引用确认和孤儿文件清理 worker 的首个实现切片。
