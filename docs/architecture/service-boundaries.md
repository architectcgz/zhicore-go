...
在对应 Go 服务完全替换 Java 服务前，`../zhicore-microservice` 仍是接口和行为的事实源；迁移目标不规划 Java/Go 运行时并存。
...
- `zhicore-id-generator` 不作为当前默认核心依赖，只作为 Java 迁移映射和未来集中发号需求的可选落点。
...
6. 保留 Java 外部 API 形态；前端暂时不修改，当前开发阶段不做灰度，Gateway 只能做路由或环境切换，不能把 API 形态变化传递给前端。
