...
| `ListFollowerShard` | 给 Notification fanout 分片读取粉丝；依赖不可用返回 degraded，不能以空 shard 冒充成功。 |
...
```text
事务前：
  如 avatarFileId 非空，调用 Upload/File Service 校验文件存在、图片类型、状态可引用
...
| 下游 Upload/Auth 不可用 | 502 / 503 | 项目通用下游不可用错误 |
