...
| `ListFollowerShard(audienceClass, activeSince, cursor, limit)` | Notification fanout 分片读取活跃粉丝 | Notification | 内部 cursor 分页；发文 fanout 默认 `audienceClass=HOT`；依赖不可用返回 `SERVICE_DEGRADED`，不能把空 shard 当成功，也不能从 `HOT` 静默 fallback 到 `ALL`。 |
...
Notification fanout 的 `ListFollowerShard` 使用服务间内部 cursor，不复用前端 cursor。User provider 拥有 `HOT` / `WARM` / `COLD` / `ALL` 受众分层和 `activeSince` 的解释；Notification 第一阶段只请求 `HOT` 活跃粉丝并物化这些用户的收件箱。`ALL` 只用于受控 backfill、repair 或管理任务，不作为普通发文默认路径。
