...
前端 HTTP 响应应返回 `avatarUrl`，但 `avatarUrl` 是 Upload/File Service 派生展示字段，不落库、不进 User 事件、不进默认 typed client。
...
`BatchGetUserSimple` 只返回 `avatarFileId`，不返回 `avatarUrl`。需要展示 URL 的调用方批量走 Upload/File Service 解析，或由面向前端的 owning service 自己解析。
...
- 非空时，User 写入前同步调用 Upload/File Service 校验文件存在、类型为图片且状态可引用。
...
| Upload/Auth 下游不可用 | 502 / 503 | 项目通用下游不可用错误 |
