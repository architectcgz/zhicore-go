...
| `ListFollowerShard` | service route + `X-Caller-Service` | `zhicore-notification` + operation + shard cursor bucket；限制 fanout worker 并发和分页速率。 | fanout job retry / DLQ；不能返回空 shard 冒充成功。 |
...
- Notification `ListFollowerShard` 是高 fanout 读路径；Redis 限流不可用时优先让 job retry / DLQ，而不是无限回源 PostgreSQL。
...
所有阈值、窗口、burst、冷却时间、批量大小桶、内部服务调用配额、Notification fanout 并发和 Redis 故障 fallback 时长必须配置化，不能写死在 handler 或 application 中。
