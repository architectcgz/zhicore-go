...
- 管理评论文本、图片文件引用、语音文件引用和语音时长。
- 管理评论点赞关系、回复数、点赞数读模型、点赞计数 delta 和统计修正。
...
- 文件事实归 Upload / File Service，Comment 图片和语音只保存系统内媒体文件 ID；可展示 / 可播放 URL 由查询时解析或缓存派生。
...
| Comment Stats | 回复数、点赞数读模型和统计修正；点赞数由 delta worker 批量更新 | `comment_stats`、`comment_counter_deltas` |
| Comment Hot Rank | 顶级评论 HOT 排序读模型，避免高频查询大范围 join 排序 | `comment_hot_rank` |
| Comment Query | 详情、分页、游标分页、增量补拉和管理端查询 | `comments`、`comment_stats`、`comment_hot_rank`、Redis cache |
