...
| --- | --- | --- |
| `CreateComment` / `CreateReply` | Content、User、User relation、Upload 或 PostgreSQL 不可用 | 失败 `1004`，不创建评论、不写 outbox。 |
...
- Application test 覆盖写路径下游不可用 fail closed：Content、User relation、Upload 校验失败时不写 comment、不写 outbox；更新评论必须覆盖 Content guard 不可用失败。
