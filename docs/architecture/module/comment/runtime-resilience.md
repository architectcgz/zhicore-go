...
- 写路径前置 guard 不能降级放行：Content、User、User relation、Upload 校验不可确认时，不写入本地评论事实。
- 查询路径允许展示增强降级：作者摘要失败可返回占位作者，Upload URL 解析失败可省略 URL；但 `publicId` 只能来自 User 或本地已确认快照，文章可见性、评论存在性和登录态不能靠降级伪造。
...
| --- | --- | --- | --- | --- | --- |
| Content `CheckPostCommentable(postId)` | 创建 / 更新 / 公开读取前校验文章可见性 | `500ms` 到 `1s` | 只读 guard 可最多 2 次总尝试，带 jitter | 开启 | 写路径和可见性校验 fail closed，返回 `1004`；不基于旧页面状态放行。 |
...
| Upload `ValidateFileReferences` | 创建 / 更新评论媒体 guard | `500ms` 到 `1s` | 最多 2 次总尝试 | 开启 | fail closed，返回 `1004` 或媒体校验错误；不能保存未确认文件引用。 |
| Upload `BatchResolveFileURLs` | 列表 / 详情展示 URL | `300ms` 到 `800ms` | 最多 2 次总尝试 | 开启 | 可省略 `imageUrls` / `voiceUrl`，保留 `imageFileIds` / `voiceFileId`。 |
...
| --- | --- | --- |
| `CreateComment` / `CreateReply` | Content、User、User relation、Upload 或 PostgreSQL 不可用 | 失败 `1004`，不创建评论、不写 outbox、不消耗 `floor`。 |
| `UpdateComment` | Content、User、User relation、Upload 或 PostgreSQL 不可用 | 失败 `1004`；不能在文章、权限和媒体事实不可确认时改内容。 |
...
| `ListTopLevelComments` / `GetCommentDetail` | Upload URL 解析不可用 | 返回文件 ID，省略 URL 字段；记录 degraded metric。 |
...
| `zhicore-upload` | 文件引用校验 | 写路径 fail closed。 |
| `zhicore-upload` | URL 解析 | 查询路径省略 URL。 |
...
- 必要配置已加载，例如 Content/User/Upload client base URL、RabbitMQ/Redis 配置格式。
...
- Application test 覆盖写路径下游不可用 fail closed：Content、User relation、Upload 校验失败时不写 comment、不写 outbox、不消耗 floor；更新评论必须覆盖 Content guard 不可用失败。
- Application test 覆盖查询降级：作者摘要不可用返回占位作者，`publicId` 缺失时仅在 `unavailable=true` 下省略，Upload URL 解析失败时保留 file ID 并省略 URL。
