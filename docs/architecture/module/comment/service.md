...
| Use case | 职责 |
| --- | --- |
| `CreateComment` | 校验文章和作者事实，分配文章内 `floor`，创建根评论或回复，写 `comments`、初始化 `comment_stats`、回复时更新根评论回复数，写 `comment.created` outbox。 |
...
重复点赞、取消未点赞按目标 contract 幂等成功，不重复写事件。
...
| 点赞 / 取消点赞 | 更新或删除点赞数、用户点赞状态、目标评论详情、所属列表和首页快照。 |
