...
- File：校验 `imageFileIds` / `voiceFileId` 存在、类型匹配且状态可用。

Content / User / File service 不可用时写请求失败，不降级写入本地评论事实。查询路径可以对作者摘要和媒体 URL 解析做降级。完整 timeout、retry、熔断和降级矩阵见 `runtime-resilience.md`。
...
| 下游 Content / User / File service / Ranking 不可用 | `1004` |
| 缺少登录态 | `2006` |
| 需要管理员角色 | `2007` |
| 非作者修改或删除评论 | `2008` |
