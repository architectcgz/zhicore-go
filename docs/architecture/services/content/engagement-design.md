...
点赞 / 收藏命令必须在单个 PostgreSQL 事务内完成关系表、统计和 outbox 记录。事务提交后 best-effort 更新 Redis；Redis 失败不回滚业务事务。
...
4. 如果关系已存在，幂等成功，不重复增加统计，不发布重复事件。
5. 如果关系首次创建，原子增加 `post_stats.like_count`，写 `content.post.liked` outbox 事件。
...
4. 如果关系不存在，幂等成功，不重复减少统计，不发布事件。
5. 如果关系被删除，原子减少 `post_stats.like_count`，写 `content.post.unliked` outbox 事件。
6. 事务提交后 best-effort 更新 Redis 状态和计数缓存。

收藏 / 取消收藏与点赞 / 取消点赞同构，只替换关系表、统计字段和事件类型。
...
- `PUT /like`、`DELETE /like`、`PUT /favorite`、`DELETE /favorite` 返回确定状态和最新计数。
...
- 重复点赞幂等成功，不重复增加 `like_count`，不重复写 outbox。
- 重复取消点赞幂等成功，不把 `like_count` 减成负数，不重复写 outbox。
