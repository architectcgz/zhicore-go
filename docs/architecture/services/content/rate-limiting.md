...
Gateway 不能替代 Content 业务限流。Gateway 不知道文章 owner、`postId`、草稿保存节奏、presence session、内部服务调用方和 outbox retry 目标。
...
当前 `<TYPE>` 覆盖 `PUBLIC_READ`、`DRAFT_WRITE`、`PUBLISH_LIFECYCLE`、`ENGAGEMENT_WRITE`、`ENGAGEMENT_READ`、`ADMIN_COMMAND` 和 `INTERNAL_CLIENT`。默认值为：`public_read=120/min local_memory`、`draft_write=30/min local_memory`、`publish_lifecycle=5/min fail_closed`、`engagement_write=60/min local_memory`、`engagement_read=120/min local_memory`、`admin_command=10/min fail_closed`、`internal_client=120/min fail_closed`。
