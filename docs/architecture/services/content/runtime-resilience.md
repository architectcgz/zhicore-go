...
| `redis` | `rate_limit.check` | Content 业务限流 | `100ms..300ms` | 不重试放大延迟；可短本机 fallback | `redis.rate_limit.check` | 独立于缓存连接池 | 按 `rate-limiting.md` 决定 `ALLOW` / `1003` / `1004` | 高副作用路径不能 fail-open。 |
...
- Redis rate limit 不可用时，`rate-limiting.md` 中 fail-open / fail-closed 分支分别可测。
