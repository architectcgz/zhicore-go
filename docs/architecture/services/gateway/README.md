...
-> Gateway 写入 `X-User-Id` / `X-User-Name` / `X-User-Roles`
...
- Gateway 转发前必须清理或覆盖客户端伪造的 `X-User-Id`、`X-User-Name` 和 `X-User-Roles`；身份 header 以 Gateway 注入值为准。
...
- 固定 Gateway 注入给下游服务的身份 header：`X-Request-Id`、`X-Trace-Id`、`X-User-Id`、`X-User-Name`、`X-User-Roles`。
