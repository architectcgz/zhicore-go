...
- Upload 文件元数据、对象存储路径或 URL 生成规则；Comment 只保存系统内媒体文件 ID，创建 / 更新时校验引用，展示 / 播放 URL 由读取时解析或缓存派生。Comment 不提供媒体上传 facade。
...
| 媒体 | Comment 不提供媒体上传 facade；前端直接调用 Upload 后把文件 ID 写入评论 | 无 Comment endpoint |
...
| Upload | 校验评论图片和语音文件引用，查询时解析展示 / 播放 URL。 |
