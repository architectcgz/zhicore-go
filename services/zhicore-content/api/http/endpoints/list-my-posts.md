...
状态：已验证。本文固定作者工作台列表入口，已由 application / repository / handler test 覆盖可信 `X-User-Id`、status、cursor、limit、envelope 和 PostgreSQL metadata 查询。
...
列表只读取 PostgreSQL metadata，不批量读取 MongoDB 正文。
