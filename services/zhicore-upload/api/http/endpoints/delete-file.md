...
| --- | --- |
| 方法 | `DELETE` |
| 主路径 | `/api/v1/upload/file/{fileId}` |
| 兼容别名 | 无 |
| Content-Type | 无 body |
| 鉴权 | 匿名 / 服务间；不读取 `Authorization` 或 `X-User-Id` |
| 幂等 | 当前 contract 不保证幂等；由 File Service adapter 决定重复删除语义 |
...
`400` / `500` 是当前 Go handler 实际输出的兼容例外。外部 File Service 返回文件不存在、无权限、删除失败或不可用时，adapter 后续必须映射为服务级 README 登记的稳定错误。

## 权限和可见性

- 本 endpoint 当前不校验业务 owner，调用方必须在业务服务中先确认文件引用可删除。
- Upload 不拥有头像、封面、评论媒体等业务引用关系，不负责级联清理业务实体。
- 删除 File Service 失败时不提供本地补偿队列；需要补偿必须先更新 Upload 服务设计。
...
| 事务边界 | 无本地事务；删除副作用在外部 File Service。 |
| 事件 | 当前不发布事件。 |
