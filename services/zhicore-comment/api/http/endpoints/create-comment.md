...
| --- | --- | --- | --- |
| `1001` | `400` | `Invalid request` | `postId` 格式非法、`parentFloor` 非正数、`imageFileIds` 数量超过 9、文件 ID 格式非法、`voiceDuration` 非法、图片和语音同时存在。 |
...
| `5001` | `404` | `Comment not found` | `parentFloor` 指向的评论已删除或对当前用户不可见。 |
| `5003` | `400` | `Comment content is required` | 文本、图片、语音整体为空。 |
| `5004` | `400` | `Comment content is too long` | `content` 超过 2000 字。 |
| `5005` | `404` | `Root comment not found` | `parentFloor` 指向的根评论或其根评论不存在 / 已删除。 |
| `5006` | `404` | `Parent comment not found` | `parentFloor` 指向的评论不存在 / 已删除。 |
...
| Ports | `ContentPostClient`、`UserProfileClient`、`UserRelationClient`、`FileReferenceClient`、`RateLimiter`、`CommentFloorAllocator`、`CommentCommandRepository`、`CommentStatsRepository`、`CommentPostStatsRepository`、`OutboxPublisher`、`TransactionRunner`、`Clock` |
| 事务边界 | 本地父评论 / 根评论树校验通过后分配 `floor`，写 `comments`、初始化统计、维护 `comment_post_stats`、根评论初始化 `comment_hot_rank` / `comment_recommended_rank`、回复时递增根评论 `reply_count`、写 `comment.created` outbox 在同一 PostgreSQL 事务内完成。 |
...
- Application test：待补，覆盖 `floor` 分配、根评论创建、回复创建、统计初始化、文章级统计、outbox 写入和缓存失效语义。
- System HTTP test：待补。
