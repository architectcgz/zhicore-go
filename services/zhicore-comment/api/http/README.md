...
- Go contract test：`services/zhicore-comment/api/http/*_handler_test.go`
...
Comment API 是 Go-first 设计。外部评论定位使用 `(postId, commentId)`：

- `postId` 是 Content 对外文章 ID。
- `commentId` 是 Comment 对外评论 ID，由内部 `comments.id BIGINT IDENTITY` 派生。
- HTTP path 不直接暴露内部 `comments.id` 数字。
- Comment 本地持久化保存 Content 公开 `postId` 字符串，并保存 Content 内部 `post_id BIGINT` opaque reference 供事件下游使用；HTTP contract 不暴露内部 `post_id`。
...
- 运行期 timeout、retry、熔断、降级和观测：见 `docs/architecture/runtime-operations.md`、`docs/architecture/observability.md` 和 `docs/architecture/module/comment/runtime-resilience.md`。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `publicId` | string | 条件必填 | User 对外公开用户 ID；正常作者摘要必须返回。仅当 `unavailable=true` 且 User 摘要不可用、Comment 又没有已确认本地快照时可省略，不能伪造。 |
...
| 方法 | 路径 | 文档 | 状态 |
| --- | --- | --- | --- |
| `POST` | `/api/v1/posts/{postId}/comments` | `endpoints/create-comment.md` | 已验证 |
| `GET` | `/api/v1/posts/{postId}/comments/page` | `endpoints/list-comments-page.md` | 已验证 |
| `GET` | `/api/v1/posts/{postId}/comments/{commentId}` | `endpoints/get-comment-detail.md` | 已验证 |
| `GET` | `/api/v1/posts/{postId}/comments/{commentId}/replies/page` | `endpoints/list-replies-page.md` | 已验证 |
| `DELETE` | `/api/v1/posts/{postId}/comments/{commentId}` | `endpoints/delete-comment.md` | 已验证 |
| `POST` | `/api/v1/posts/{postId}/comments/{commentId}/like` | `endpoints/like-comment.md` | 已验证 |
| `DELETE` | `/api/v1/posts/{postId}/comments/{commentId}/like` | `endpoints/unlike-comment.md` | 已验证 |
| `GET` | `/api/v1/posts/{postId}/comments/{commentId}/liked` | `endpoints/get-like-status.md` | 已验证 |
...
| `PUT` | `/api/v1/posts/{postId}/comments/{commentId}` | API 族已识别 |
| `GET` | `/api/v1/posts/{postId}/comments/{commentId}/replies/cursor` | API 族已识别 |
| `GET` | `/api/v1/posts/{postId}/comments/{commentId}/replies/incremental` | API 族已识别 |
| `GET` | `/api/v1/posts/{postId}/comments/{commentId}/like-count` | API 族已识别 |
| `POST` | `/api/v1/posts/{postId}/comments/batch/liked` | API 族已识别 |
| `GET` | `/api/v1/admin/comments` | API 族已识别 |
| `DELETE` | `/api/v1/admin/comments/posts/{postId}/comments/{commentId}` | API 族已识别 |
...
| --- | --- | --- | --- | --- |
| `POST /api/v1/posts/{postId}/comments` | `CreateComment` / `CreateReply` | `docs/architecture/module/comment/service.md` | 已验证 | `go test ./api/http` |
| `GET /api/v1/posts/{postId}/comments/page` | `ListTopLevelCommentsByPage` | `docs/architecture/module/comment/service.md` | 已验证 | `go test ./api/http` |
| `GET /api/v1/posts/{postId}/comments/{commentId}` | `GetCommentDetail` | `docs/architecture/module/comment/service.md` | 草案 | 待补 |
| `GET /api/v1/posts/{postId}/comments/{commentId}/replies/page` | `ListRepliesByPage` | `docs/architecture/module/comment/service.md` | 草案 | 待补 |
| `DELETE /api/v1/posts/{postId}/comments/{commentId}` | `DeleteComment` | `docs/architecture/module/comment/service.md` | 草案 | 待补 |
| `POST /api/v1/posts/{postId}/comments/{commentId}/like` | `LikeComment` | `docs/architecture/module/comment/service.md` | 草案 | 待补 |
| `DELETE /api/v1/posts/{postId}/comments/{commentId}/like` | `UnlikeComment` | `docs/architecture/module/comment/service.md` | 草案 | 待补 |
| `GET /api/v1/posts/{postId}/comments/{commentId}/liked` | `GetLikeStatus` | `docs/architecture/module/comment/service.md` | 草案 | 待补 |
...
| `1004` | `503` | `Service unavailable` | Content、User、File service、Ranking、PostgreSQL 或 Redis 不可用。 |
...
| `5001` | `404` | `Comment not found` | 评论、目标评论 ID 或已删除评论对普通用户不可见。 |
