...
状态：已验证。本文固定 `DELETE /api/v1/users/{publicId}/follow` 字段级 contract，已由 Go handler / contract test 验证。
...
| 幂等 | 未关注时重复取消返回成功，不发事件 |

## Path 参数

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `publicId` | string | 是 | 取消关注目标用户的公开 ID。 |
