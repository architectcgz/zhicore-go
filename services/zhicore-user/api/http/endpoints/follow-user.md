...
状态：已验证。本文固定 `POST /api/v1/users/{publicId}/follow` 字段级 contract，已由 Go handler / contract test 验证。
...
| --- | --- |
| 方法 | `POST` |
| 主路径 | `/api/v1/users/{publicId}/follow` |
| 兼容别名 | 无 |
| Content-Type | 无 body |
| 鉴权 | 登录用户，`X-User-Id` 必填 |
| 幂等 | 重复关注返回成功，不重复统计或事件 |

## Path 参数

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `publicId` | string | 是 | 被关注用户的公开 ID。 |
...
- 目标用户由 `publicId` 解析，关系表只保存内部 `userId`。
- 新增关注要求双方都是 `ACTIVE` 且任一方向不存在拉黑关系。
