...
状态：已验证。本文固定 `POST /api/v1/users/{publicId}/block` 字段级 contract，已由 Go handler / contract test 验证。
...
| 幂等 | 重复拉黑返回成功，不重复事件或统计副作用 |

## Path 参数

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `publicId` | string | 是 | 被拉黑用户的公开 ID。 |
...
- 首次拉黑会在同一事务内删除双方关注关系并写关系事件；解除拉黑不恢复关注。
