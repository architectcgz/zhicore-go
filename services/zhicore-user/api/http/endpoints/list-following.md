...
状态：已验证。本文固定 `GET /api/v1/users/{publicId}/following` 字段级 contract，已由 Go handler / contract test 验证。
...
| --- | --- |
| 方法 | `GET` |
| 主路径 | `/api/v1/users/{publicId}/following` |
| 兼容别名 | 无 |
| Content-Type | 无 body |
| 鉴权 | 匿名 / 登录用户 |
| 幂等 | 查询接口，天然幂等 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `publicId` | string | 是 | 被查看关注列表的目标用户公开 ID。 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `items` | `UserProfileResp[]` | 是 | 被关注用户摘要。 |
