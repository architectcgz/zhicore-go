...
状态：已验证。本文固定 `GET /api/v1/users/me/blocked` 字段级 contract，已由 Go handler / contract test 验证。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `items` | `UserProfileResp[]` | 是 | 被当前用户拉黑的用户摘要。 |
...
- Cursor 分页，按 `user_blocks.id DESC` 稳定排序。
