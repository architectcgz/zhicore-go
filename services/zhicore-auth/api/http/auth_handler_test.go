...
func TestRegisterSuccessSetsCookiesAndReturnsAuthenticatedPrincipal(t *testing.T) {
	...
	if !refreshCookie.Expires.Equal(refreshExpiresAt) {
		t.Fatalf("refresh cookie expires = %s, want %s", refreshCookie.Expires, refreshExpiresAt)
	}
...
func TestLoginSuccessPassesRememberMeAndSetsCookies(t *testing.T) {
	now := time.Date(2026, 7, 4, 12, 0, 0, 0, time.UTC)
	...
	if !service.loginInput.RememberMe {
		t.Fatal("login rememberMe = false, want true")
	}
}
...
func TestLoginRejectsMissingRememberMe(t *testing.T) {
	service := &fakeAuthService{}
	req := jsonRequest(t, http.MethodPost, "/api/v1/auth/login", map[string]any{
		"email":    "user@example.com",
		"password": "Password123",
	...
	if service.loginCalled {
		t.Fatal("login service must not be called when rememberMe is missing")
	}
}
