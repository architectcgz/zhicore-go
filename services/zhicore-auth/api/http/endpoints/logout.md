...
| `X-CSRF-Token` | 条件必填 | 请求携带 refresh cookie 并需要服务端撤销 session 时必须提交。 |
...
| `csrf_token` | 条件必填 | 与 `X-CSRF-Token` 做 double-submit 校验。 |
...
- Handler contract test：待补，覆盖 access + refresh、仅 refresh、无凭证清 cookie、`202 PROCESSING`、CSRF 失败。
- System HTTP test：待补。
